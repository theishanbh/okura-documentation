...
// , => to exclude url like www.bing,,com
const httpExcludeRegEx =
  /^[^?]+%[^0-9a-f]+|^[^?]+%[0-9a-f][^0-9a-f]+|^[^?]+%00|^[^?]+%$|^https?:\/\/[^?\/]+@|^www\.[^?\/]+@/i;
...
const linkMatchRules: Record<string, LinkMatchRule> = {
  http: {
    ...
    normalizeUrl: (url) =>
      new RegExp("^(?:microsoft-edge:)?http:\\/\\/", "i").test(url)
        ? url
        : "http://" + url,
  },
  https: {
    match: new RegExp(
      `^(?:microsoft-edge:)?https:\\/\\/${domainPortWithUrlRegEx}`,
      "i"
    ),
    except: httpExcludeRegEx,
  },
  ...
  ftp: {
    ...
    normalizeUrl: (url) =>
      new RegExp("^ftp:\\/\\/", "i").test(url) ? url : "ftp://" + url,
  },
  news: { match: new RegExp(`^news:(\\/\\/)?${domainPortWithUrlRegEx}`, "i") },
  telnet: {
    match: new RegExp(`^telnet:(\\/\\/)?${domainPortWithUrlRegEx}`, "i"),
  },
  ...
  if (url) {
    for (const schema of getObjectKeys(linkMatchRules)) {
      ...
      if (
        matches &&
        matches[0] == url &&
        (!rule.except || !rule.except.test(url))
      ) {
        return {
          scheme: schema,
          originalUrl: url,
