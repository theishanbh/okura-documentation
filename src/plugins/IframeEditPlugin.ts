...
import {
  ...
  updateImageEditInfo,
} from "./utils/updateImageEditInfo";
...
export class ImageEditPlugin implements ImageEditor, EditorPlugin {
    ...
    if (this.isEditing) {
      if (event.rawEvent.key === "Escape") {
        ...
        this.applyFormatWithContentModel(
          editor,
          this.isCropMode,
          (isModifierKey(event.rawEvent) || event.rawEvent.shiftKey) &&
            isImageSelection //if it's a modifier key over a image, the image should select the image
        );
      }
    }
  }
          ...
          if (
            ...
            mutateSegment(
              ...
              (image) => {
                ...
                this.imageEditInfo = updateImageEditInfo(
                  image,
                  selection.image
                );
  ...
  private startEditing(
    ...
    this.imageHTMLOptions = getHTMLImageOptions(
      editor,
      this.options,
      this.imageEditInfo
    );
    const { resizers, rotators, wrapper, shadowSpan, imageClone, croppers } =
      createImageWrapper(
        editor,
        image,
        this.options,
